...
Rust は、この問題を型システムのレベルで解決します。「値が存在しないかもしれない」という可能性を、実行時ではなくコンパイル時にプログラマに強制的に意識させるのです。その中心的な役割を果たすのが、列挙型 (`enum`) と、その代表例である `Option<T>` です。
...
ここまでは他の言語と似ていますが、Rust の `enum` の真価は、各バリアントがそれぞれ異なるデータを持てることにあります。
